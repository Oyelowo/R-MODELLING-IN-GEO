...
for (i in 1:rep){
  ...
  #GAM
  phyc_gam<-gam(Phycae~s(mesotopo, k=3) + s(soil_moist, k=3) + s(soil_temp, k=3) + 
  ...
  #GBM1
  phyc_gbm1<-gbm(formula = Phycae~mesotopo+soil_moist+soil_temp+soil_ph, data=cal,
  ...
  #GBM2 dismo
  phyc_gbm2<-gbm(formula = Phycae~mesotopo+soil_moist+soil_temp+soil_ph, data=cal,
                 distribution = "bernoulli",n.trees = 3000, shrinkage = 0.001, interaction.depth = 4)
  ...
  phyc_auc_gbm_p2<-colAUC(pred_phyc_gbm2, eva$Phycae, plotROC = F)
...
compared_model_phyc
colMeans(compared_model_phyc)

fpa <- "C:/Users/oyeda/Desktop/MODELLING_PHYSICAL_GEOGRAPHY/final_group/validation_tables/Species"
write.csv(compared_model_phyc, file=paste0(fpa,"/AUC_VACV_1.csv"))

write.csv(compared_model_phyc, file=paste0(fpa,"/MEAN_aUC_VACV.csv"))
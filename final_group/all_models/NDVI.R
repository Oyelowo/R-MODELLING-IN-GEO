...
#check cryoturbation script for the correkation matrix and ggpairs
#
...
#note: dont use auc for poissonn and gaussian because it is meant for binary variables.

ndviM_glm <- glm(NDVI ~Altitude + I(Altitude^2) + Radiation+
                     I(Radiation^2)+Tavg_7+ I(Tavg_7^2) + RR_annual+I(RR_annual^2)+ Slope +  I(Slope^2), data=data, family = "gaussian")
...
# 
# #final model for ndviMocesses
ndviM_glm <- glm(NDVI ~RR_annual + Slope + 
                     I(Slope^2), data=data, family = "gaussian")
#test for significance of the predictors.
anova (ndviM_glm, test="Chisq")   #F or Chisq for gaussian. Chisq for gaussian and binomial
...
# gam_ndviM <- gam(NDVI ~ s(RR_annual, k=3)+ s(Slope, k=3), data = data, family = "gaussian")
...
#               distribution = "gaussian",n.trees = 10000, shrinkage = 0.001, interaction.depth = 6)
# 
...
#   cal <- data[-i,]
#   eva <-data[i,]
#   ndviM_glm <- glm(NDVI ~ RR_annual+ Slope +  I(Slope^2), data=cal, family = "gaussian") 
#   
#   
...
for (i in 1:rep){
  ...
  colnames(me_rmse_ndviM_glm)<- c("Cryo_glm")
  
  
  
  
  #GAM
  ndviM_gam <- gam(NDVI ~ s(RR_annual, k=3)+ s(Slope, k=3), data = cal, family = "gaussian")
  ...
  colnames(me_rmse_ndviM_gam)<- c("Cryo_gam")
  ...
  #using the normal gbm, package.
  #GBM
  ndviM_gbm1<-gbm(formula = NDVI ~Altitude + RR_annual + Radiation+ Slope+Tavg_7, data=cal,
                    distribution = "gaussian",n.trees = 300, shrinkage = 0.01, interaction.depth = 6,
                    bag.fraction = 0.75)
  ...
  colnames(me_rmse_ndviM_gbm1)<- c("Cryo_gbm1")
  ...
  ndviM_gbm2 <-gbm.step(data=cal, gbm.x =c("Altitude","RR_annual","Radiation","Slope","Tavg_7"), gbm.y = "Cryoturb",
                          bag.fraction=0.75, learning.rate = 0.001,family="gaussian",n.trees=50, n.folds=10,
                          max.trees = 10000, tree.complexity = 6)
  #best.iter<-gbm.perf(ndviM_gbm1, plot.it = T, method = "OOB")
  ...
  colnames(me_rmse_ndviM_gbm2)<- c("Cryo_gbm2")
...
rownames(all_error_ndviM)<- c("mean error", "RMSE")

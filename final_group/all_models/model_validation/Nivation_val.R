...
# 
# 
# #final model for nivaturbation
# niva_glm <- glm(Nivation~ RR_annual+ Slope +  I(Slope^2), data=data, family = "poisson") 
...
for (i in 1:rep){
  ...
  niva_glm <- glm(Nivation~  Tavg_7+ I(Tavg_7^2)+ Slope
                  +  I(Slope^2), data=cal, family = "poisson") 
  ...
  colnames(me_rmse_niva_glm)<- c("niva_glm")
  
  
  
  
  #GAM
  niva_gam <- gam(Nivation~ s(Tavg_7, k=2)+ s(Slope, k=2)+ s(Radiation, k=2), data = cal, family = "poisson")
  ...
  colnames(me_rmse_niva_gam)<- c("niva_gam")
  ...
  #using the normal gbm, package.
  #GBM
  niva_gbm1<-gbm(formula = Nivation~ Tavg_7 + NDVI  + Slope, data=cal,
                 distribution = "poisson",n.trees = 2800, shrinkage = 0.001, interaction.depth = 6,
                 bag.fraction = 0.75)
  
  best.iter<-gbm.perf(niva_gbm1, plot.it = F, method = "OOB")
  ...
  colnames(me_rmse_niva_gbm1)<- c("niva_gbm1")
  ...
  niva_gbm2 <- gbm.step(data=cal, gbm.x =
  ...
  #The below is an alternative method of predicting here
  ...
  colnames(me_rmse_niva_gbm2)<- c("niva_gbm2")
...
rownames(all_error_niva)<- c("mean abs error", "RMSE")

}

fpa <- "C:/Users/oyeda/Desktop/MODELLING_PHYSICAL_GEOGRAPHY/final_group/validation_tables"
write.csv(all_cor_niva, file=paste0(fpa,"/correlation_Nivation.csv"))

write.csv(all_error_niva, file=paste0(fpa,"/error_Nivation.csv"))

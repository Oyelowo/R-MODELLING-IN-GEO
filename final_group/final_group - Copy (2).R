...
for (i in 1:rep){
  ...
  #                              type="response")
  cryo_gbm2_pred<- predict(object=cryo_gbm2,
                           model=cryo_gbm2, fun=predict,
                           n.trees=cryo_gbm2$n.trees, type="response")
  cryo_pred_gbm2_ras <- predict(object=ras_stack,
                           model=cryo_gbm2, fun=predict,
                           n.trees=cryo_gbm2$n.trees, type="response")
...
plot.glm(cryo_gbm2_pred)
plot.gam(cryo_gbm2_pred)
plot(cryo_gbm1_pred, best.iter)
plot.gbm(cryo_gbm2_pred)

best.iter <- gbm.perf(cryo_gbm1_pred,method="OOB",plot.it=TRUE)

plot(cryo_gbm1_pred,1,best.iter) 

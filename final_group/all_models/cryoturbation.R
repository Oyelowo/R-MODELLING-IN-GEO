...
for (i in 1:rep){
  ...
  #GAM
  cryo_gam <- gam(Cryoturb~ s(RR_annual, k=2)+ s(Slope, k=2), data = cal, family = "poisson")
  ...
  #using the normal gbm, package.
  #GBM
  cryo_gbm1<-gbm(formula = Cryoturb~ RR_annual + Slope, data=cal,
...
all_cor_cryo <- cbind.data.frame(cor_glm_cryo,cor_gam_cryo,
  ...
  rownames(all_error_cryo)<- c("mean abs error", "RMSE")
  
}

fpa <- "C:/Users/oyeda/Desktop/MODELLING_PHYSICAL_GEOGRAPHY/final_group/validation_tables"
write.csv(all_cor_cryo, file=paste0(fpa,"/correlation_Cryoturb.csv"))

write.csv(all_error_cryo, file=paste0(fpa,"/error_Cryoturb.csv"))
